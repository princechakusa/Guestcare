...
window.grOpenEditProperty = function(id) {
  console.log('grOpenEditProperty called with id:', id);
  const p = store.properties.find(x => x.id == id);
  console.log('Found property:', p);
  if (!p) return;
  grPropertyModal = 'edit';
  grEditId = id;
...
window.grOpenEditReview = function(id) {
  console.log('grOpenEditReview called with id:', id);
...
const VIEWS = {
  ...
  'r-first': () => {
    const fm = store.messages;
    if (!fm.length) return `<div class="panel">${emptyState('No data', 'Import messages to see first response times.')}</div>`;
    const agents = store.agents;
    const rows = agents.map(a => {
      const msgs = fm.filter(m => m.agentId == a.id && m.responded && m.rt != null);
      const first = msgs.length ? Math.min(...msgs.map(m => m.rt)).toFixed(1) : null;
      return `<tr><td><span class="pill" style="background:${a.color}22;color:${a.color}">${a.name}</span></td><td>${first ? first + 'm' : '—'}</td></tr>`;
    });
    return `<div class="panel"><div class="ph"><span class="pt">First Response Time</span></div><div class="tw"><table><thead><tr><th>Agent</th><th>First Response</th></tr></thead><tbody>${rows.join('')}</tbody></table></div></div>`;
  },
  'r-avg': () => {
    const fm = store.messages;
    if (!fm.length) return `<div class="panel">${emptyState('No data', 'Import messages to see average response times.')}</div>`;
    const agents = store.agents;
    const rows = agents.map(a => {
      const msgs = fm.filter(m => m.agentId == a.id && m.responded && m.rt != null);
      const avg = msgs.length ? (msgs.reduce((s, m) => s + m.rt, 0) / msgs.length).toFixed(1) : null;
      return `<tr><td><span class="pill" style="background:${a.color}22;color:${a.color}">${a.name}</span></td><td>${avg ? avg + 'm' : '—'}</td></tr>`;
    });
    return `<div class="panel"><div class="ph"><span class="pt">Average Response Time</span></div><div class="tw"><table><thead><tr><th>Agent</th><th>Average</th></tr></thead><tbody>${rows.join('')}</tbody></table></div></div>`;
  },
  'r-delays': () => {
    const fm = store.messages;
    if (!fm.length) return `<div class="panel">${emptyState('No data', 'Import messages to see delays.')}</div>`;
    const agents = store.agents;
    const rows = agents.map(a => {
      const msgs = fm.filter(m => m.agentId == a.id && m.responded && m.rt != null);
      const delays = msgs.filter(m => m.rt > SLA[m.channel]);
      return `<tr><td><span class="pill" style="background:${a.color}22;color:${a.color}">${a.name}</span></td><td>${delays.length}</td></tr>`;
    });
    return `<div class="panel"><div class="ph"><span class="pt">Delays Over SLA Threshold</span></div><div class="tw"><table><thead><tr><th>Agent</th><th>Delays</th></tr></thead><tbody>${rows.join('')}</tbody></table></div></div>`;
  },
  'r-missed': () => {
    const missed = store.messages.filter(m => !m.responded);
    if (!missed.length) return `<div class="panel">${emptyState('No missed messages', 'All conversations have responses.')}</div>`;
    return `<div class="panel"><div class="ph"><span class="pt">Missed / No Response</span></div><div class="tw"><table><thead><tr><th>Guest</th><th>Agent</th><th>Channel</th><th>Last Message</th><th>Time</th></tr></thead><tbody>${missed.map(m => `<tr><td>${m.guest}</td><td>${agentName(m.agentId)}</td><td>${m.channel}</td><td>${m.message || ''}</td><td>${fmt(m.ts)}</td></tr>`).join('')}</tbody></table></div></div>`;
  },

  // ----- Satisfaction (Placeholders with simple tables) -----
  'sat-reviews': () => `<div class="panel"><div class="ph"><span class="pt">Review Scores</span></div><div class="pp">Connect to channel APIs or import review data to see scores.</div></div>`,
  'sat-complaints': () => `<div class="panel"><div class="ph"><span class="pt">Complaints</span></div><div class="pp">Track guest complaints manually or via integrations.</div></div>`,
  'sat-nps': () => `<div class="panel"><div class="ph"><span class="pt">NPS / CSAT</span></div><div class="pp">Connect NPS survey tool to display scores.</div></div>`,
  'sat-repeat': () => `<div class="panel"><div class="ph"><span class="pt">Repeat Guests</span></div><div class="pp">Requires booking history import.</div></div>`,
  'sat-resolution': () => `<div class="panel"><div class="ph"><span class="pt">Resolution Quality</span></div><div class="tw"><table><thead><tr><th>Agent</th><th>Conversations</th><th>Responded</th><th>Avg Response</th></tr></thead><tbody>${store.agents.map(a=>{const s=agentStats(a.id);return`<tr><td>${a.name}</td><td>${s.total}</td><td>${s.total-s.active}</td><td>${s.avgRt? s.avgRt+'m' : '—'}</td></tr>`}).join('')}</tbody></table></div></div>`,

  // ----- Operations (Placeholders with basic data) -----
  'ops-closed': () => `<div class="panel"><div class="ph"><span class="pt">Tickets Closed</span></div><div class="tw"><table><thead><tr><th>Agent</th><th>Total</th><th>Responded</th></tr></thead><tbody>${store.agents.map(a=>{const s=agentStats(a.id);return`<tr><td>${a.name}</td><td>${s.total}</td><td>${s.total-s.active}</td></tr>`}).join('')}</tbody></table></div></div>`,
  'ops-active': () => `<div class="panel"><div class="ph"><span class="pt">Active Issues</span></div><div class="tw"><table><thead><tr><th>Guest</th><th>Agent</th><th>Channel</th><th>Waiting Since</th></tr></thead><tbody>${store.messages.filter(m=>!m.responded).map(m=>`<tr><td>${m.guest}</td><td>${agentName(m.agentId)}</td><td>${m.channel}</td><td>${fmt(m.ts)}</td></tr>`).join('')}</tbody></table></div></div>`,
  'ops-escalations': () => `<div class="panel"><div class="ph"><span class="pt">Escalations</span></div><div class="pp">Log escalations manually.</div></div>`,
  'ops-maintenance': () => `<div class="panel"><div class="ph"><span class="pt">Maintenance</span></div><div class="pp">Track maintenance requests and response times.</div></div>`,
  'ops-inspections': () => `<div class="panel"><div class="ph"><span class="pt">Inspections</span></div><div class="pp">Log inspection scores per property.</div></div>`,
  'ops-deepclean': () => `<div class="panel"><div class="ph"><span class="pt">Deep Cleaning</span></div><div class="pp">Schedule and track deep cleans.</div></div>`,

  // ----- Channel Intel (Placeholders) -----
  'chan-airbnb': () => `<div class="panel"><div class="ph"><span class="pt">Airbnb Performance</span></div><div class="pp">Connect Airbnb API to see bookings, response rate, and CSAT.</div></div>`,
  'chan-booking': () => `<div class="panel"><div class="ph"><span class="pt">Booking.com Performance</span></div><div class="pp">Connect Booking.com API for live metrics.</div></div>`,
  'chan-direct': () => `<div class="panel"><div class="ph"><span class="pt">Direct Bookings</span></div><div class="pp">Log direct bookings manually or via Hostaway export.</div></div>`,

  // ----- Proactive (Placeholders) -----
  'pro-prearrival': () => `<div class="panel"><div class="ph"><span class="pt">Pre-arrival Outreach</span></div><div class="pp">Track % of guests who received check-in instructions.</div></div>`,
  'pro-midstay': () => `<div class="panel"><div class="ph"><span class="pt">Mid-stay Check-ins</span></div><div class="pp">Log mid-stay messages from chat imports.</div></div>`,

  // ----- System (Heatmap & Sentiment) -----
  'heatmap': () => {
    const msgs = store.messages.filter(m => m.responded && m.rt != null);
    if (!msgs.length) return `<div class="panel">${emptyState('No data', 'Import messages to see response time heatmap.')}</div>`;
    const grid = {};
    ...
    const rows = days.map((d, di) => `<div class="hm-label">${d}</div>${hours.map(h => `<div class="hm-cell" style="${cellColor(di, h)}" title="${d} ${h}:00 — avg ${cellLabel(di, h) || 'no data'}">${cellLabel(di, h)}</div>`).join('')}`).join('');
    return `<div class="panel"><div class="ph"><span class="pt">Response Time Heatmap (by hour and day)</span></div><div class="heatmap"><div class="hm-head"></div>${hours.map(h => `<div class="hm-head">${h}</div>`).join('')}${rows}</div></div>`;
  },
  'sentiment': () => {
    const msgs = store.messages;
    if (!msgs.length) return `<div class="panel">${emptyState('No data', 'Import messages to analyse guest sentiment.')}</div>`;
    const posWords = ['thank','thanks','great','excellent','perfect','amazing','wonderful','love','happy','good','clean','comfortable','nice','fantastic','brilliant','quick','fast','helpful','superb'];
    const negWords = ['dirty','broken','terrible','awful','horrible','disgusting','unacceptable','worst','bad','slow','noisy','smelly','disappointed','frustrated','angry','refund','complaint','emergency','never','problem'];
    const scoreSentiment = (text) => {
      const t = (text || '').toLowerCase().split(/\s+/);
      const pos = t.filter(w => posWords.includes(w)).length;
      const neg = t.filter(w => negWords.includes(w)).length;
      const total = pos + neg || 1;
      return { pos: Math.round(100 * pos / total), neg: Math.round(100 * neg / total), label: neg > pos ? 'Negative' : pos > neg ? 'Positive' : 'Neutral' };
    };
    const allScores = msgs.map(m => scoreSentiment(m.message));
    const posCount = allScores.filter(s => s.label === 'Positive').length;
    const negCount = allScores.filter(s => s.label === 'Negative').length;
    const neuCount = msgs.length - posCount - negCount;
    return `<div class="stats-row"><div class="stat-card"><div class="stat-label">Positive</div><div class="stat-val up">${posCount}</div></div><div class="stat-card"><div class="stat-label">Negative</div><div class="stat-val down">${negCount}</div></div><div class="stat-card"><div class="stat-label">Neutral</div><div class="stat-val">${neuCount}</div></div><div class="stat-card"><div class="stat-label">Total</div><div class="stat-val">${msgs.length}</div></div></div>
    <div class="panel"><div class="ph"><span class="pt">Sentiment by Agent</span></div><div class="pp">${store.agents.map(a => { const ms = msgs.filter(m => m.agentId == a.id); if (!ms.length) return `<div><span class="pill p-gray">${a.name}</span> No data</div>`; const scores = ms.map(m => scoreSentiment(m.message)); const pos = Math.round(scores.reduce((s, x) => s + x.pos, 0) / scores.length); const neg = Math.round(scores.reduce((s, x) => s + x.neg, 0) / scores.length); return `<div><span class="pill" style="background:${a.color}22;color:${a.color}">${a.name}</span> ${pos}% positive, ${neg}% negative (${ms.length} msgs)</div>`; }).join('')}</div></div>`;
  },

  // Guest Reviews (full implementation as before)
  'gr-properties': () => {
...
// Fill any remaining placeholders (should be none, but keep as safety)
['sat-reviews','sat-complaints','sat-nps','sat-repeat','ops-escalations','ops-maintenance','ops-inspections','ops-deepclean','chan-airbnb','chan-booking','chan-direct','pro-prearrival','pro-midstay'].forEach(v=>{
   if(!VIEWS[v]) VIEWS[v] = () => `<div class="panel"><div class="ph">${v}</div><div class="pp">Coming soon</div></div>`;
});

const LBL={overview:'Overview',agents:'Agents',import:'Import','r-first':'First Response','r-avg':'Avg Response','r-delays':'Delays','r-missed':'Missed','root-cause':'Root Cause','sat-reviews':'Reviews','sat-complaints':'Complaints','sat-nps':'NPS/CSAT','sat-repeat':'Repeat Guests','sat-resolution':'Resolution','ops-closed':'Tickets Closed','ops-active':'Active Issues','ops-escalations':'Escalations','ops-maintenance':'Maintenance','ops-inspections':'Inspections','ops-deepclean':'Deep Cleaning','gr-properties':'Properties','gr-reviews':'Reviews','gr-analysis':'Analysis','gr-import':'Import CSV','chan-airbnb':'Airbnb','chan-booking':'Booking.com','chan-direct':'Direct','pro-prearrival':'Pre-arrival','pro-midstay':'Mid-stay','heatmap':'Response Heatmap','sentiment':'Guest Sentiment',integrations:'Integrations',admin:'Admin',help:'Help'};
...
window.editAgent = function(id) {
  console.log('editAgent called with id:', id);
  const a = store.agents.find(x => x.id == id);
  console.log('Found agent:', a);
  if (!a) return;
